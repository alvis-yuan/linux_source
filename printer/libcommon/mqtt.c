...
enum MQTTErrors mqtt_sync(struct mqtt_client *client) {
    ...
    if (client->inspector_callback != NULL) {
        ...
        if( client->error == MQTT_OK ) client->error = err;
        MQTT_PAL_MUTEX_UNLOCK(&client->mutex);
        if (err != MQTT_OK) return err;
    }
    ...
    if (err != MQTT_OK){
		LogError("__mqtt_recv %s(%d)",mqtt_error_str(err),err);
		return err;
	}
    ...
    if (err != MQTT_OK){
		LogError("__mqtt_send %s(%d)",mqtt_error_str(err),err);
		return err;
	}
    return err;
}
...
enum MQTTErrors mqtt_crete_socket( struct mqtt_client *client, enum TLSVersion version, const char* addr,
        const char* port, const char* ca_file, const char* ca_path)
{
	int ret;
	MQTT_PAL_MUTEX_LOCK(&client->mutex);
	client->error =MQTT_ERROR_NULLPTR;

	//printf("%s start %d\n", __func__, version);
    if (client  == NULL || addr == NULL || port == NULL ) {
        ret = MQTT_ERROR_NULLPTR;
        goto mqtt_mutex;
    }

	if( (version < TLS0 )  || (version > TLS1_3) )
	{
		ret = MQTT_ERROR_NULLPTR;
		goto mqtt_mutex;
	}

    if ((version >  TLS1_0) && (ca_file == NULL) && (ca_path == NULL) )
    {
    	ret = MQTT_ERROR_NULLPTR;
    	goto mqtt_mutex;
    }
    ...
    if(client->socketfd)
    {
    	//printf("%s socketfb already exists\n", __func__);
    	mqtt_pal_close_socket(client->socketfd, client->tls_version);
    }
    ret = mqtt_pal_open_socket(&client->socketfd, version, addr, port, ca_file, ca_path);
    if(ret < 0)
    {
        LogError("mqtt creak socket error:%d,%s",ret, strerror(errno));
    ...
    if(version != 0)
    {
    	...
    	context = (struct tls_context *)client->socketfd;
    	printf("%s  create tls %p %p\n", __func__, context->bio, context->ssl_ctx);
    }
    else
    	printf("%s  create socket %d\n", __func__,*( int *)(client->socketfd));
	#endif
    client->error = MQTT_OK;
    client->tls_version = version;
    //printf("%s end\n", __func__);
    ret =  MQTT_OK;

mqtt_mutex:
	MQTT_PAL_MUTEX_UNLOCK(&client->mutex);
	if(client->error != MQTT_OK) client->socketfd = NULL;
	client->error = ret;
	return ret;
}

void mqtt_close_socket( struct mqtt_client *client)
{
	MQTT_PAL_MUTEX_LOCK(&client->mutex);
	if(client->socketfd)
	{
		mqtt_pal_close_socket(client->socketfd, client->tls_version);
		memset(client, 0x00, sizeof(struct mqtt_client));
...
enum MQTTErrors mqtt_get_status(struct mqtt_client *client) {
    enum MQTTErrors rv;
    MQTT_PAL_MUTEX_LOCK(&client->mutex);
    //rv = __mqtt_ping(client);
   if( (client->alives_flag) && (client->number_of_keep_alives <= 2) )
    	rv = MQTT_OK;
    else
    	rv = MQTT_ERROR_CONNECT_NOT_CALLED;
    MQTT_PAL_MUTEX_UNLOCK(&client->mutex);
    return rv;
}
...
ssize_t __mqtt_send(struct mqtt_client *client) 
{
    ...
    for(i = 0; i < len; ++i) {
        ...
        if(msg->control_type == MQTT_CONTROL_PINGREQ)
        {
        	//printf("sent ping requests to service\n");
        	client->number_of_keep_alives++;
            if(client->number_of_keep_alives > 2)
            {
            	LogError("More than two ping requests were not received");
            	client->alives_flag = 0;
            	client->error = MQTT_ERROR_CONNECT_NOT_CALLED;
            	MQTT_PAL_MUTEX_UNLOCK(&client->mutex);
        ...
        ssize_t tmp = mqtt_pal_send(client->tls_version, client->socketfd, msg->start, msg->size, 0);
        if (tmp < 0) {
            client->error = tmp;
            MQTT_PAL_MUTEX_UNLOCK(&client->mutex);
            return tmp;
...
ssize_t __mqtt_recv(struct mqtt_client *client) 
{
    ...
    while(1) {
        ...
        rv = mqtt_pal_recv(client->tls_version, client->socketfd, client->recv_buffer.curr, client->recv_buffer.curr_sz, 0);
        if (rv < 0) {
            /* an error occurred */
            LogError("mqtt_pal_recv fail(%d)",rv);
            client->error = rv;
            MQTT_PAL_MUTEX_UNLOCK(&client->mutex);
            return rv;
        } else {
        ...
        if (consumed < 0) {
            ...
            if (client->recv_buffer.curr_sz == 0) {
                ...
                MQTT_PAL_MUTEX_UNLOCK(&client->mutex);
				LogError("MQTT_ERROR_RECV_BUFFER_TOO_SMALL");
        ...
        switch (response.fixed_header.control_type) {
...
#else
					LogWarn("MQTT_CONTROL_CONNACK of unknown id:%u",response.decoded.pubrel.packet_id);
					break;
#endif
                }
                msg->state = MQTT_QUEUED_COMPLETE;
            ...
            case MQTT_CONTROL_PUBACK:
                ...
                if (msg == NULL) {
                    ...
                    return MQTT_ERROR_ACK_OF_UNKNOWN;
#else
					LogWarn("MQTT_CONTROL_PUBACK of unknown id:%u",response.decoded.pubrel.packet_id);
					break;
#endif
                }
                msg->state = MQTT_QUEUED_COMPLETE;
            ...
            case MQTT_CONTROL_PUBREC:
                ...
                if (msg == NULL) {
                    ...
                    return MQTT_ERROR_ACK_OF_UNKNOWN;
#else
					LogWarn("MQTT_CONTROL_PUBREC of unknown id:%u",response.decoded.pubrel.packet_id);
#endif
                } else {
					msg->state = MQTT_QUEUED_COMPLETE;
                	/* update response time */
            ...
            case MQTT_CONTROL_PUBREL:
                ...
                if (msg == NULL) {
                    ...
                    return MQTT_ERROR_ACK_OF_UNKNOWN;
#else
					LogWarn("MQTT_CONTROL_PUBREL of unknown id:%u",response.decoded.pubrel.packet_id);
#endif
                } else {
					msg->state = MQTT_QUEUED_COMPLETE;
                	/* update response time */
            ...
            case MQTT_CONTROL_PUBCOMP:
                ...
                if (msg == NULL) {
                    ...
                    return MQTT_ERROR_ACK_OF_UNKNOWN;
#else
					LogWarn("MQTT_CONTROL_PUBCOMP of unknown id:%u",response.decoded.pubrel.packet_id);
					break;
#endif
                }
                msg->state = MQTT_QUEUED_COMPLETE;
            ...
            case MQTT_CONTROL_SUBACK:
                ...
                if (msg == NULL) {
                    ...
                    return MQTT_ERROR_ACK_OF_UNKNOWN;
#else
					LogWarn("MQTT_CONTROL_SUBACK of unknown id:%u",response.decoded.pubrel.packet_id);
					break;
#endif
                }
                msg->state = MQTT_QUEUED_COMPLETE;
            ...
            case MQTT_CONTROL_UNSUBACK:
                ...
                if (msg == NULL) {
                    ...
                    return MQTT_ERROR_ACK_OF_UNKNOWN;
#else
					LogWarn("MQTT_CONTROL_UNSUBACK of unknown id:%u",response.decoded.pubrel.packet_id);
					break;
#endif
                }
                msg->state = MQTT_QUEUED_COMPLETE;
            ...
            case MQTT_CONTROL_PINGRESP:
                ...
                if (msg == NULL) {
                    ...
                    return MQTT_ERROR_ACK_OF_UNKNOWN;
#else
					LogWarn("MQTT_CONTROL_PINGRESP of unknown id:%u",response.decoded.pubrel.packet_id);
					break;
#endif
                }
                msg->state = MQTT_QUEUED_COMPLETE;
