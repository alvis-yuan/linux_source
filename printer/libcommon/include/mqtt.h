...
#define __ALL_MQTT_ERRORS(MQTT_ERROR)                    \
	...
	MQTT_ERROR(MQTT_ERROR_NET_CHANGE)
...
/**
 * @brief Initializes an MQTT socket.
 * @ingroup api
 *
 * This function \em must be called before any other API function calls.
 *
 * addr: MQTT service address
 * port: MQTT service port
 * ca_file:  Mqtt service ca certificate path string
 * ca_path: Mqtt service ca certificate directory string
 *when version > TLS1_0   ca_file and ca_pach can't be empty,
 *
 ...
 * @ingroup api
  * @pre mqtt_crete_socket must have been called.
 *
 * @param[in,out] client The MQTT client.
 *
 * @note To re-establish the session, mqtt_crete_socket must be called.
 *
 * @pre None.
 */
