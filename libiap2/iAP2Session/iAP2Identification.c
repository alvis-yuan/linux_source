...
#define g_eaProtocalName "com.example.protocol"
 ...
 * 添加EA协议组参数
 */
static int _AddEAProtocolGroup(ctrlSessBuilder* builder, const iAP2EAProtocol_t* protocol)
{
 ...
 * 添加蓝牙传输组件组参数
 */
static int _AddBTTransportGroup(ctrlSessBuilder* builder, const iAP2BTTransportInfo_t* btInfo)
{
    ctrlSessGroupBuilder groupBuilder;
...
/* 添加USB Host传输组件组参数 */
static int _AddUSBHostTransportGroup(ctrlSessBuilder* builder, const iAP2USBHostTransportInfo_t* usbInfo)
{
    ctrlSessGroupBuilder groupBuilder;
    
    if (ctrlSess_BeginGroup(&groupBuilder, builder, kiAP2IdParamUsbHostTransComp) != 0) {
        return -1;
    }
...
static BOOL _SendIdentificationInfo(void)
{
    ...
    if (g_idConfig.messagesReceivedByAccessory && g_idConfig.messagesReceivedCount > 0) {
    ...
    ctrlSess_AddEnum(&builder, kiAP2IdParamPowerProviding, info->PowerProvidingCapability);

    /* 最大电流 (可选) */
    iAP2LogDbg("[Id] MaximumCurrentDrawnFromDevice: %u mA", info->MaximumCurrentDrawnFromDevice);
    ctrlSess_AddUint16(&builder, kiAP2IdParamMaxCurrDrawn, info->MaximumCurrentDrawnFromDevice);
...
int iAP2IdInit(const iAP2IdConfig_t* config, uint8_t type)
{
    ...
    g_idConfig.messagesSentCount = sizeof(g_messagesSent) / sizeof(g_messagesSent[0]);
    g_idConfig.messagesReceivedByAccessory = g_messagesReceived;
    g_idConfig.messagesReceivedCount = sizeof(g_messagesReceived) / sizeof(g_messagesReceived[0]);
    
    g_idState = kIAP2IdStateIdle;
    g_initialized = TRUE;
    
    iAP2LogDbg("[Id] Identification module initialized with built-in config");
    iAP2LogDbg("[Id] Accessory: %s (%s)", g_accessoryInfo.name, g_accessoryInfo.modelIdentifier);
...
int iAP2IdSendUpdate(const iAP2AccessoryInfo_t* updatedInfo)
{
    ...
    ctrlSessBuilder builder;
    ctrlSess_BuilderInit(&builder, g_msgBuffer, ID_MSG_BUFFER_SIZE, kiAP2IdMsgUpdate);
    ...
    if (updatedInfo->currentLanguage) {
        ctrlSess_AddString(&builder, kiAP2IdParamCurrentLang, updatedInfo->currentLanguage);
    }
                            ...
                            const char* fwVersion, const char* hwVersion)
{
    ...
    if (g_idState != kIAP2IdStateIdle && g_idState != kIAP2IdStateWaitStart) {
        iAP2LogError("[Id] Cannot change accessory info in current state: %d", g_idState);
        return -1;
    }
...
int iAP2IdSetBluetoothMACName(const uint8_t* macAddress, const char* name)
{
    ...
    if (name) {
        snprintf(g_btTransport.transportName, sizeof(g_btTransport.transportName), "%s", name);
    }
