...
int iAP2SendRequestAppLaunch(const char* bundleId, uint8_t launchMethod)
{
    ...
    ctrlSess_BuilderInit(&builder, g_msgBuffer, CTRL_MSG_BUFFER_SIZE, kiAP2AppMsgLaunchReq);
                                      ...
                                      uint16_t listMax)
{
    ...
    ctrlSessBuilder builder;
    ctrlSess_BuilderInit(&builder, g_msgBuffer, CTRL_MSG_BUFFER_SIZE, kiAP2AppDisMsgStartUpdate);
    
    /* 添加Categories参数（可选） */
    if (categories && categoryCount > 0) {
        ctrlSessGroupBuilder groupBuilder;
        
        if (ctrlSess_BeginGroup(&groupBuilder, &builder, kiAP2AppDisParamCategories) != 0) {
...
int iAP2SendStopAppDiscoveryUpdates(void)
{
    ...
    ctrlSessBuilder builder;
    ctrlSess_BuilderInit(&builder, g_msgBuffer, CTRL_MSG_BUFFER_SIZE, kiAP2AppDisMsgStopUpdate);
...
int iAP2SendEAPSessionStatus(uint16_t sessionId, uint8_t status)
{
    ...
    ctrlSessBuilder builder;
    ctrlSess_BuilderInit(&builder, g_msgBuffer, CTRL_MSG_BUFFER_SIZE, kiAP2EapMsgSessionStatus);
...
static int iAP2HandleEAPStartSessionInternal(uint16_t sessionId, uint8_t protocolId)
{
    if (!g_ctrlMsgCtx.initialized) {
        iAP2LogError("[CtrlMsg] Module not initialized");
        return -1;
