...
static void _HandleIdentifyResult(BOOL accepted, uint8_t rejectReason, void* context);
static BOOL _SendControlMessage(const uint8_t* data, uint32_t len, void* context);
                                  ...
                                  uint8_t sessionID, void* context)
{
    ...
    iAP2LogDbg("%s session %d received %d bytes", sessionType == kIAP2SessionTypeEA ? "EA" : "Control", sessionID, dataLen);
...
static void _HandleLinkConnected(BOOL connected, void* context)
{
    ...
    if (connected) {
        ...
        iAP2Link_t* link = iAP2TransportGetLink(session->transport);
        if (link) {
            iAP2PacketSessionInfo_t* ctrlSessInfo = iAP2LinkGetSessionInfo(link, kIAP2CtrlSessionId);
            if (ctrlSessInfo) {
                iAP2LogDbg("[Session] Negotiated control session version: %d, sessNum=%d", ctrlSessInfo->version, link->param.numSessionInfo);
...
static BOOL _SendControlMessage(const uint8_t* data, uint32_t len, void* context)
{
    ...
    iAP2PacketSessionInfo_t* sessInfo = iAP2LinkGetSessionInfo(link, kIAP2CtrlSessionId);
    if (!sessInfo) {
...
iAP2Session_t* iAP2SessionCreate(const iAP2SessionConfig_t* config, uint8_t type)
{
    ...
    authConfig.controlSessionVersion = 1;  /* 默认版本1，Link连接后会更新 */
